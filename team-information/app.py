...
def setPage(member_id):
    member = db.members.find_one({'_id': ObjectId(member_id)})  # DB에서 해당 멤버 데이터를 가져옴
...
def member_edit(member_id):
    try:
        ...
        return jsonify({"message": f"{member_id}번 팀원 수정 성공"})
...
def member_delete(member_id):
    try:
        ...
        return jsonify({"message": f"{member_id}번 팀원 삭제 성공"})
